...
var resizePizzas = function(size)
{
  ...
  console.log("size from resizePizzas ", size);
  ...
  function changeSliderLabel(size)
  {
    ...
    {
      case "1":
      // changed from document.querySelector("#pizzaSize") to document.getElementById('pizzaSize')
  ...
  {
    //moved size determination outside the loop
...
function updatePositions()
{
  ...
  //moved the prephase calc outside the for loop
  var prephase = (document.body.scrollTop / 1250);
  //moved phase modulo calc outside the main loop  - calculations stored in phaseMod array
  var phaseMod=[];
  for (k=0; k<5;k++)
  {
    phaseMod[k]=(k % 5);
  ...
  for (var i = 0; i < items.length; i++)
    {
      ...
      var phase = (Math.sin((prephase) + (phaseMod[k])));
      //pizza movement
      var moveLeft = 100 * phase + 'px';
      items[i].style.transform = "translateX("+moveLeft+")";
...
var makeSliders = function()
{
  ...
  //use the size of the client window to determine the number of pizzas
  // remove - var moveTop = (Math.floor(i / cols) * s) + 'px';
  for (var j = 0; j < numberOfPizzas; j++)
    {
      var elem = document.createElement('img');
      elem.className = 'mover';
      //hide backface Visibility to improve rendering
      elem.style.WebkitBackfaceVisibility = "hidden";
      elem.src = "images/pizza.png";
      elem.style.height = "100px";
      elem.style.width = "73.333px";
      elem.style.left = (j % cols) * s +"px";
...
document.addEventListener('DOMContentLoaded', makeSliders());
